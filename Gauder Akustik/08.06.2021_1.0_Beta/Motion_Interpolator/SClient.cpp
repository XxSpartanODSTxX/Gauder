...
int  writeSocket(SOCKET ConnectSocket, const char* sendText) {
    char sendbuf[DEFAULT_BUFLEN];
    int iResult;
    sprintf_s(sendbuf, "%s", sendText);
...
int availableSocket(SOCKET ConnectSocket)
{
    int iResult = 0;
    unsigned long bytes_available = 0;;
    iResult= ioctlsocket(ConnectSocket, FIONREAD, &bytes_available);
    return bytes_available;
}

int __cdecl xxxxx_main(int argc, char** argv)
{
  //  WSADATA wsaData;
 //   SOCKET ConnectSocket = INVALID_SOCKET;
...
//    const char* servername = "localhost";
    ...
    */
    ...
    for (long nindex = 0; nindex < 2000000000; nindex++) {
        ...
        if (nAnswers > 0) {
            nAnswers--;
            iResult = writeSocket( ConnectSocket, sendText);
     ...
     //   mySleep(0, 1000);
      ...
      //  do {
        ...
        iResult = readSocket(ConnectSocket, buffer);
        if (iResult > 0)
            nAnswers = 5;
        /*
        ioctlsocket(ConnectSocket, FIONREAD, &bytes_available);
