...
int doCommands(char c, char sub, int icmd) {
  ...
  switch (c) {
    ...
    case 'I': //Motion In
      ...
      Motion.reset(axis1, axis2); //Actual Value  position
      Motion.moveIn();
      Motion.movebSpline();
    ...
    case 'R': //Reset Motor Values
      ...
      Serial.println("Disable Operational Mode"); //tiene que ser (1,1)o (1,2)
      canOpen.setOperationalMode(0, 1);
      delay(5);
      canOpen.setOperationalMode(0, 2);
      delay(5);
      Serial.println("Finish Operational Mode is Disable");
      arrayIndex = 0;
      break;
...
int moveTarget(char sub, int icmd) {
  ...
  switch (sub) {
    case 's':  // Teach Targert   Target 16 or OutTarget is last saved Target
      Motion.setFinalTarget(icmd, axis1, axis2, 0.3, 1);
      break;
    case 'g': // Turn Targert to absolut Position (without using invers Kinematik)
      ...
      axis2 +=  icmd * M_PI / 180.0;  // Set orientation Angle to absolut value
      Motion.setFinalTarget(Motion.outTarget, axis1, axis2, 0, 0);
    ...
    case 'h': // Turn Targert to absolut Position
      ...
      axis1 = Motion.getOutTarget(0);
      axis2 = Motion.getOutTarget(1);
      Motion.reset(getDrivePosition(0x601), getDrivePosition(0x602));
      if (Robot.getTCP() == 0) {
        posTheta[0] = axis1;
        posTheta[1] = axis2 + icmd * M_PI / 180.0;  // Set orientation Angle to absolut value
      }
      else {
        posWorld = Robot.jointWorld(axis1, axis2);
        posWorld[2] = axis2 + icmd * M_PI / 180.0; // Set orientation Angle to absolut value
    ...
    case 'z':
      //*********New***********//
      if (scmd.fdata[0]) {
        canOpen.setOffset(scmd.fdata[0], 0x601);
        delay(15);
      }
      if (scmd.fdata[1]) {
        canOpen.setOffset(scmd.fdata[1], 0x602);
        delay(15);
      }
      break;
    case 'm':
